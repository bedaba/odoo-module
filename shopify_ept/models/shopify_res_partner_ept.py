...
class ShopifyResPartnerEpt(models.Model):
    ...
    def shopify_create_contact_partner(self, vals, instance, queue_line, log_book):
        ...
        partner_values = vals.get("default_address", {})
        phone = partner_values.get("phone", "")
        if email or phone:
            partner = partner_obj.search_partner_by_email_phone(email, phone)
    ...
    def shopify_create_or_update_address(self, shopify_customer_data, parent_partner, partner_type="contact"):
        ...
        if not partner:
            partner = partner_obj._find_partner_ept(
                partner_vals, address_key_list,
        ...
        if partner:
            return partner
        if partner_vals.get('mobile') and parent_partner.mobile == partner_vals.get('mobile'):
            partner_vals.pop('mobile')
        if partner_vals.get('phone') and parent_partner.phone == partner_vals.get('phone'):
            partner_vals.pop('phone')
        partner_vals.update({
            "type": partner_type, "parent_id": parent_partner.id
        })
        email = partner_vals.get("email", "")
        phone = partner_vals.get("phone", "")
        if email or phone:
            partner = partner_obj.search_partner_by_email_phone(
                email, phone, get_parent=False
            )
            if partner:
